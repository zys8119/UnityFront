<template>
    <div class="Tree" :class="{isTop:isTop,lazyLoading:!lazyLoading}">
        ...
        <slot name="expand" :vm="_self"></slot>
        <div class="TreeBox">
            <TreeItem
                    @draggable="(...args)=>draggable(args[0],level,parentNode,args[1],args[2])"
                    :draggableMove="draggableMove"
                    :options="currentOptions" :isdraggable="isdraggable && draggableFilter(_self)">
                ...
                transition:`height ${atf} ${attime}ms`,
                    ...
                    level:level,
                    checked:checkedMap === ((keyMap)?`${keyMap},${key}`:`${key}`),
                }" :self="_self">{{item[showNameField]}}</slot>
                    </div>
    ...
    export default {
        ...
        methods: {
            ...
            initOptions() {
                ...
                if (this.optionsAll && this.$store.state.layout.TreeOpenedKeys && this.$store.state.layout.TreeOpenedKeys.length > 0) {
            ...
            toggleClick(item, key, node) {
                ...
                // 获取节点打开的keys
                this.resetOpenKeys();
                if (this.optionsAll) this.getOpenKeys(this.optionsAll, null);
            },
            // 获取节点打开的keys
            getOpenKeys(arr, k) {
                arr.forEach((a, b) => {

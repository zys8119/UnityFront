<template>
    <span @click="showMap">
            ...
            <div class="amap-page-conpont" v-if="show" :class="{relative:relative}">
                ...
                <div class="toolbar">
                    ...
                    <div class="map-btn-group">
                        <el-button type="button" class="map-btn map-btn-cancel" @click.stop="handleCancel(level)">取消</el-button>
                        <el-button type="button" class="map-btn map-btn-submit" @click.stop="handleSure(level)" :disabled="btnStatus">确定
                        </el-button>
                    </div>
                </div>
                <!--河道搜索-->
        ...
        <ul class="draggable" @click.stop="()=>{}" v-if="level === 1 && !relative">
            <draggable v-model="address" :options="{animation:500}">
                <li class="address-list-items" v-for="(item,index) in address" :key="index">
...
<script>
    import region from "./data/NingboRegion"

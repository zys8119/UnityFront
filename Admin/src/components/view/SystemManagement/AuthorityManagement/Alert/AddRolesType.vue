...
export default {
    ...
    methods:{
        // 保存
        save(){
            if(this.$utils.is_S(this.formData.name)){return  this.$message.error("请输入角色类型名称")}
            if(this.$utils.is_S(this.formData.type)){return  this.$message.error("请选择分类")}
            if(this.row){
                this.apis.AuthorityManagement.RolesType.update(this.formData).then(()=>{
            ...
            this.apis.AuthorityManagement.RolesType.add(this.formData).then(()=>{
                this.$message({type:"success",message:"保存成功"})
                this.$emit("save")
                this.$ZAlert.hide()
            })
